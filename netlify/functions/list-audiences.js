// Read-only: audience/interest criteria applied at ad-group level — in-market &
// affinity interests (USER_INTEREST), remarketing lists (USER_LIST), custom &
// combined audiences, and the newer combined AUDIENCE resource. Shows whether
// each is targeting or observation/excluded.
//
// GET /.netlify/functions/list-audiences?customerId=9427798225[&campaignId=...]   (x-mcc-token required)
...
exports.handler = async (event) => {
  ...
  let gaql = `
    SELECT campaign.name, ad_group.id, ad_group.name,
      ad_group_criterion.criterion_id, ad_group_criterion.type, ad_group_criterion.status, ad_group_criterion.negative,
    ...
    WHERE ad_group_criterion.type IN ('USER_INTEREST','USER_LIST','CUSTOM_AUDIENCE','COMBINED_AUDIENCE','AUDIENCE')
  `;
  ...
  try {
    ...
    const audiences = rows.map((r) => {
      ...
      const ref =
        c.userInterest?.userInterestCategory ||
        c.userList?.userList ||
        c.customAudience?.customAudience ||
        c.combinedAudience?.combinedAudience ||
        c.audience?.audience || null;
      return {
        criterion_id: c.criterionId,
        kind: c.type,
        reference: ref, // resource name / id of the interest, list, or audience
        excluded: !!c.negative,
        status: c.status,
    ...
    return json(200, { ok: true, version: env.version, customerId, count: audiences.length, audiences });
  } catch (e) {
