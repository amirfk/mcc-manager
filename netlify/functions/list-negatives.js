// Read-only: existing campaign-level NEGATIVE keywords — so we can see what's
// already blocked before adding more (avoids duplicates, shows coverage gaps).
//
// GET /.netlify/functions/list-negatives?customerId=9427798225[&campaignId=...]   (x-mcc-token required)
...
exports.handler = async (event) => {
  ...
  let gaql = `
    SELECT campaign.id, campaign.name,
      campaign_criterion.criterion_id, campaign_criterion.keyword.text, campaign_criterion.keyword.match_type
  ...
  try {
    ...
    const negatives = rows.map((r) => ({
      ...
      text: r.campaignCriterion?.keyword?.text || null,
      match_type: r.campaignCriterion?.keyword?.matchType || null,
      campaign_id: r.campaign?.id || null,
      campaign_name: r.campaign?.name || null,
    }));

    return json(200, { ok: true, version: env.version, customerId, count: negatives.length, negatives });
  } catch (e) {
